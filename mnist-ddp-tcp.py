...
def train(gpu, args):
    # rank = args.nr * args.gpus + gpu # Identify the sequence number of the current process in all processes
    print('start init process group')
    print('local rank: {}'.format(gpu))
    ...
    model = nn.parallel.DistributedDataParallel(model, device_ids=[gpu], output_device=gpu, broadcast_buffers=False)
    # Data loading code
